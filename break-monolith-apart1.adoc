...
----
mvn quarkus:add-extension -Dextensions="hibernate-orm-panache, jdbc-h2, smallrye-health" -f $PROJECT_SOURCE/cloud-native-workshop-v2m1-labs/inventory
----
...
[source,sh,role="copypaste"]
----
mvn quarkus:dev -Dquarkus.http.host=0.0.0.0 -f $PROJECT_SOURCE/cloud-native-workshop-v2m1-labs/inventory
----
...
INFO  [io.quarkus] (Quarkus Main Thread) Installed features: [agroal, cdi, hibernate-orm, hibernate-orm-panache, jdbc-h2, narayana-jta, resteasy, resteasy-jsonb, smallrye-context-propagation, smallrye-health]
...
VS Code will also detect that the Quarkus app opens port `5005` (for debugging) and `8080` (for web requests). Do not open port 5005, but open in the new tab for port `8080`, which opens a small web browser in VS Code:

image::open-port-1.png[Inventory RESTful Service, 700]

You should see the inventory web frontend directly in VS Code (you may need to click the _reload_ icon):

image::inventory-codeready.png[Inventory RESTful Service, 700]

Open a *new* VS Code Terminal:

image::codeready-workspace-terminal-2.png[Inventory RESTful Service, 700]
...
Add _quarkus-jdbc-postgresql_ and _openshift_ extension via VS Code Terminal:

[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="jdbc-postgresql,openshift" -f $PROJECT_SOURCE/cloud-native-workshop-v2m1-labs/inventory
----
...
mvn clean package -DskipTests -f $PROJECT_SOURCE/cloud-native-workshop-v2m1-labs/inventory
----
