...
Quarkus is a _Kubernetes Native_ Java stack, crafted from the best of breed Java libraries and standards. Amazingly fast boot time, incredibly low RSS memory (not just heap size!) offering near instant scale up and high density memory utilization in container orchestration platforms like Kubernetes. Quarkus uses a technique called compile time boot. https://quarkus.io/vision/container-first[Learn more^].

This will be one of the runtimes included in https://www.redhat.com/en/products/runtimes[Red Hat Runtimes^].
...
----
2020-03-09 14:41:17,171 INFO  [io.agr.pool] (main) Datasource '<default>': Initial size smaller than min. Connections will be created when necessary
2020-03-09 14:41:17,454 INFO  [io.quarkus] (main) inventory 1.0-SNAPSHOT (running on Quarkus 1.2.1.Final) started in 3.353s. Listening on: http://0.0.0.0:8080
2020-03-09 14:41:17,457 INFO  [io.quarkus] (main) Profile dev activated. Live Coding activated.
2020-03-09 14:41:17,457 INFO  [io.quarkus] (main) Installed features: [agroal, cdi, hibernate-orm, hibernate-orm-panache, jdbc-h2, narayana-jta, resteasy, resteasy-jsonb, smallrye-health]
----
...
https://access.redhat.com/documentation/en-us/red_hat_jboss_middleware_for_openshift/3/html/red_hat_java_s2i_for_openshift/index[Red Hat OpenJDK Container Image^], providing foundational software needed to run Java applications, while staying at
a reasonable size.
====
...
Updating the readiness probe caused another re-deployment of the app. Wait for this redeployment to complete by running the following command and waiting for `replication controller "inventory-quarkus-xxxx" successfully rolled out`:
...
Open the http://inventory-quarkus-{{ USER_ID }}-inventory.{{ ROUTE_SUBDOMAIN}}[Inventory UI^].
...
[source,sh,role="copypaste"]
----
oc set probe dc/inventory-quarkus --readiness --initial-delay-seconds=5 --period-seconds=5 --failure-threshold=15
----
