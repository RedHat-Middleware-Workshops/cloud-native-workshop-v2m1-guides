...
----
cd $CHE_PROJECTS_ROOT/cloud-native-workshop-v2m1-labs && git checkout ocp-4.3
----
...
on *Add*. Go the *Server Path* tab and enter the path to the fixed project at `/opt/solution` and click *Upload* to add the project:
...
To make the OpenShift console display a bit more pleasing, add labels to each app:
...
oc annotate dc/coolstore app.openshift.io/vcs-ref=ocp-4.3 --overwrite
----

There, that's better:

image::coolstore-dev-labels.png[coolstore-dev, 800]
